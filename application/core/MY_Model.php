...
class MY_Model extends CI_Model{
	...
	public function get_all_details_distinct($field = '', $table = '', $condition = '', $sortArr = '',$limitArr='') {
		if ($field != '') {
			$this->db->distinct();
		...
		if($limitArr !=''){
			$data = $this->db->get_where($table,$condition,$limitArr['l1'], $limitArr['l2']);
		}else{
			$data = $this->db->get_where($table,$condition);
		}
		return $data;
	}
	public function get_all_details_distinct_query($field = '',$table = '',$condition=''){
		if ($field != '') {
			$select_qry = "SELECT distinct ".$field." FROM ".$table." ".$condition;
		}else{
			$select_qry = "SELECT * FROM ".$table." ".$condition;
		}
		$productList = $this->db->query($select_qry);
		return $productList;
	}
	public function get_all_details_field($field = '',$table = '', $condition = '', $sortArr = '',$limitArr='') {
		if ($field != '') {
			if (is_array ( $field )) {
				$str = '';
	...
	public function get_all_details_field_query($field = '',$table = '',$condition=''){
		if ($field != '') {
			if (is_array ( $field )) {
				$str = '';
				foreach ( $field as $column ) {
				...
				$select_qry = "SELECT ".trim($str)." FROM ".$table." ".$condition;
			}else{
				$select_qry = "SELECT ".$field." FROM ".$table." ".$condition;
			}
		}else{
			$select_qry = "SELECT * FROM ".$table." ".$condition;
		}
	...
	public function get_all_details_query($table = '',$condition='',$limitArr=''){
		if(!empty($limitArr)){
			$select_qry = "SELECT * FROM ".$table." ".$condition.' limit '.$limitArr['l1'].','.$limitArr['l2'];
		}else{
			$select_qry = "SELECT * FROM ".$table." ".$condition;
		}
		$productList = $this->db->query($select_qry);
	...
	public function get_all_details_with_join_query_new($table,$table1,$on,$condition,$tb1 = '',$tb2 = '',$limitArr=''){
		if(!empty($limitArr)){
			if(!empty($tb1) && !empty($tb2)){
				$select_qry = "SELECT ".$tb1.",".$tb2." FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition.' limit '.$limitArr['l1'].','.$limitArr['l2'];;
			}else if(!empty($tb1) && empty($tb2)){
				$select_qry = "SELECT ".$tb1." FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition.' limit '.$limitArr['l1'].','.$limitArr['l2'];;
			}else{
				$select_qry = "SELECT ".$tb2.".* FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition.' limit '.$limitArr['l1'].','.$limitArr['l2'];;
			}
			
		}else{
			if(!empty($tb1) && !empty($tb2)){
				$select_qry = "SELECT ".$tb1.",".$tb2." FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition;
			}else if(!empty($tb1) && empty($tb2)){
				$select_qry = "SELECT ".$tb1." FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition;
			}else{
				$select_qry = "SELECT ".$tb2.".* FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition;
			}

			//$select_qry = "SELECT ".$table.".* FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition;
		}
		$productList = $this->db->query($select_qry);
		return $productList;
	}
	public function get_all_details_with_join_query($table,$table1,$on,$condition,$limitArr=''){
		if(!empty($limitArr)){
			$select_qry = "SELECT ".$table.".* FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition.' limit '.$limitArr['l1'].','.$limitArr['l2'];;
		}else{
			$select_qry = "SELECT ".$table.".* FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition;
		}
		$productList = $this->db->query($select_qry);
	...
	public function get_all_details_with_wishlist_query($table,$table1,$on,$condition){
		$select_qry = "SELECT ".$table.".* FROM ".$table." INNER JOIN ".$table1.' on '.$on.$condition;
		$productList = $this->db->query($select_qry);
		return $productList;
	}
	public function get_all_details_with_wishlist($table,$table1, $user_id, $sortArr = '',$limitArr='') {
		$this->db->select ( $table.'.*' );
		$this->db->where ( $table1 . '.user_id', '"'.$user_id.'"' );
		$this->db->from ( $table1 );
		$this->db->join ( $table, $table . '.id = ' . $table1 . '.product_id' );
		if ($sortArr != '' && is_array ( $sortArr )) {
			foreach ( $sortArr as $sortRow ) {
				if (is_array ( $sortRow )) {
					$this->db->order_by ( $sortRow ['field'], $sortRow ['type'] );
	...
	public function get_all_details_with_seller($table,$table1, $condition = '', $sortArr = '',$limitArr='') {
		$t = 'user_shop';
		$this->db->select ( $table.'.*,'.$table1.'.shop_id,'.$t.'.name as shop_name,'.$table1.'.profilephoto');
		if (is_array ( $condition )) {
			foreach ( $condition as $r => $v ) {
				$this->db->where ( $r, $v );
			}
		}
		$this->db->from ( $table );
		$this->db->join ( $table1, $table1 . '.id = ' . $table . '.user_id' );
		$this->db->join ( $t, $t . '.id = ' . $table1 . '.shop_id' );
		if ($sortArr != '' && is_array ( $sortArr )) {
			foreach ( $sortArr as $sortRow ) {
				if (is_array ( $sortRow )) {
					$this->db->order_by ( $sortRow ['field'], $sortRow ['type'] );
	...
	public function get_all_details_with_orderlist($table,$table1, $user_id, $sortArr = '',$limitArr='') {
		
		$t = 'payment_type_list';
		$t1 = 'offers';
		$this->db->select ( $table.'.*,'.$table1.'.payment_status,'.$table1.'.order_status,'.$table1.'.payment_mode_id,'.$t.'.name as payment_mode_name,'.$table1.'.currency,'.$table1.'.bill_first_name,'.$table1.'.bill_last_name,'.$table1.'.bill_address,'.$table1.'.bill_phone,'.$table1.'.bill_state,'.$table1.'.bill_city,'.$table1.'.bill_zipcode,'.$table1.'.bill_country,'.$table1.'.total_order_price,'.$table1.'.delivery_charge');
		$this->db->where ( $table1 . '.user_id', $user_id );
		$this->db->from ( $table );
		$this->db->join ( $table1, $table1 . '.id = ' . $table . '.orders_id' );
		$this->db->join ( $t, $t . '.id = ' . $table1 . '.payment_mode_id' );
		//$this->db->join ( $t1, $t1 . '.id = ' . $table . '.offer_id' ,'left');
		if ($sortArr != '' && is_array ( $sortArr )) {
			foreach ( $sortArr as $sortRow ) {
				if (is_array ( $sortRow )) {
					$this->db->order_by ( $sortRow ['field'], $sortRow ['type'] );
	...
	public function get_all_details_with_order($table, $table1, $user_id, $order_id) {
		$t = 'payment_type_list';
		$t1 = 'offers';
		$this->db->select ( $table.'.*,'.$table1.'.payment_status,'.$table1.'.order_status,'.$table1.'.payment_mode_id,'.$t.'.name as payment_mode_name,'.$table1.'.currency,'.$table1.'.bill_first_name,'.$table1.'.bill_last_name,'.$table1.'.bill_address,'.$table1.'.bill_phone,'.$table1.'.bill_state,'.$table1.'.bill_city,'.$table1.'.bill_zipcode,'.$table1.'.bill_country,'.$table1.'.total_order_price,'.$table1.'.delivery_charge');
		$this->db->from ( $table );
		...
		//$this->db->join ( $t1, $t1 . '.id = ' . $table . '.offer_id' ,'left');
		$this->db->where ( $table1 . '.user_id', $user_id );
		$this->db->where ( $table . '.orders_id', $order_id );
		$result = $this->db->get();
	...
	function commonUpdate($tbl_name='', $param=array(), $condition = ''){ 		
		$data  = $this->db->update($tbl_name,  $param, $condition);
		if($data){
			return true;
		}else{
			return false;
	...
	public function resize($imageFolder,$dirImage,$filename, $width, $height) {

		if (!is_file($dirImage . $filename) || substr(str_replace('\\', '/', realpath($dirImage . $filename)), 0, strlen($dirImage)) != $dirImage) {
			return;
		}
		...
		if (!is_file($dirImage . $image_new) || (filectime($dirImage . $image_old) > filectime($dirImage . $image_new))) {
			list($width_orig, $height_orig, $image_type) = getimagesize($dirImage . $image_old);
				 
			if (!in_array($image_type, array(IMAGETYPE_PNG, IMAGETYPE_JPEG, IMAGETYPE_GIF))) { 
				return $dirImage . $image_old;
			}
			...
			foreach ($directories as $directory) {
				...
				if (!is_dir($dirImage . $path)) {
					@mkdir($dirImage . $path, 0777);
				}
			}

			if ($width_orig != $width || $height_orig != $height) {
				$image = new Image($dirImage . $image_old);
				$image->resize($width, $height);
				$image->save($dirImage . $image_new);
			} else {
				copy($dirImage . $image_old, $dirImage . $image_new);
			}
		}


			return site_url(). $imageFolder.'/'. $image_new;
...
//    function modified by jeevan singh
    
    function custom_pagination($rows_per_page, $total_rows = NULL, $page_number = null,$pagination_url = ''){
        $this->load->helper('url');
        $segments = $this->uri->total_segments();
        $uri_array = $this->uri->segment_array();
...
//        $page = $this->uri->segment($segments);
        ...
        if(isset($total_rows))
        {
            if($total_rows!=0)
            {
                ...
                for ($i = 1; $i <= $number_of_pages; $i++) {
                    unset($uri_array[$segments]);
                    if($i == 1){
                        $links .= '<li class=""><a href="'.$pagination_url.'">1</a></li>';
                    }else{
                    $uri_string = implode('/', $uri_array);
                    $links .= $this->pagination_delimiters[0];
                    $links .= (($page_number == $i) ? anchor($pagination_url, $i) : anchor($pagination_url . '/' . $i, $i));
                    $links .= $this->pagination_delimiters[1]; 
                    }
    ...
    public function get2($table_name, $where = [], $offset = 0, $is_single = false, $is_total = false){
    ...
    }	public function save_data($table, $data)	{		$this->db->insert($table, $data);		$this->db->last_query();        		return $this->db->insert_id();	}
	public function delete_data($table, $where)	{		$this->db->where($where);		$this->db->delete($table);	}	/*public function get_data($table, $where)    {        $this->db->select(*);        $this->db->where($where);        $query = $this->db->get($table)->row();        $this->db->last_query();        if (($query)) {            return $query;        } else {            return false;        }        return true;    }*/
    /*
    ...
    */
    ...
    function push_notification($user_mobile_info, $payload_info=array()) {
		$api_key = PUSH_NOTIFICATION_ACCESS_KEY;
        $registrationIds = $user_mobile_info;

        if(is_array($registrationIds)){
			$fields = array(
						'registration_ids' => $registrationIds,
						'data' => $payload_info,
						'notification' => $payload_info,
						'notification_type' => $payload_info['notification_type'],
						'target_id' => $payload_info['target_id'],
						'target_url' => $payload_info['target_url'],
						'priority'=>'high'
					);
		}else{
			$fields = array(
