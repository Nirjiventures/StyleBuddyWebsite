	...
	<div class="mt-1">
      ...
      <form name="frm" action="<?=base_url($url1.'/'.$url2.'/')?>">
      	...
      	<div class="row"> 
			...
			<?php $aaa = array('1'=>'Social','2'=>'Web');?>
			<div class="col-sm-3">
				<label class="control-label" for="social_id">Register Type</label>
				<select class="form-control" name="social_id" id="social_id" >
					<option  value="">Select </option>
					<?php if($aaa) { foreach($aaa as $key=>$value) { ?>
						<?php 
						if($_GET['social_id'] && $_GET['social_id'] == $key){$sel='selected';}else{$sel='';}
						?>
						<option value="<?=$key?>" <?=$sel?>><?=$value?></option>
					<?php } } ?>
				</select>
				<?php echo form_error('social_id','<span class="text-primary mt-1">','</span>') ;?>
				<div id="social_id_err"></div>
			</div>
