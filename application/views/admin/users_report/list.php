                ...
                if (!empty($list)) {
                    ...
                    foreach ($list as $row) {
                        ...
                        <tr class="even"  data-val="<?=$row->id?>">
            				...
            				<td><?php echo !empty($row->to_user->fname) ? $row->to_user->fname : 'N/A' ?></td>
                            ...
                            <?php if($this->session->userdata('admin_id') == 1){ ?>
                                <td><button type="button" id="<?= $row->to_user->id; ?>" class="status_checks_user btn btn-sm mt-1 <?= ($row->to_user->status == 1)?"btn-primary":"btn-danger"; ?> ">
                                       <?= ($row->to_user->status == 1)?"Publish":"Blocked"; ?>
                                       </button>
                                </td>
                            <?php }?>
