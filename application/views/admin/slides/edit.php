         ...
         <div class="card  form-card">
            ...
            <div class="row">
               ...
               <div class="col-sm-6 mt-4">
                  <div class="row">
                     <div class="col-md-9">
                        <label for="Image Alt Description" class=" col-form-label">Slide Image(Upload size Max 2 MB)</label>
                        <input type="file" name="slides_image" id="slides_image" class="form-control" title="Enter product image" placeholder="Add Catogery image" >
                        <span class="text-center text-danger mb-2" id="errid"> <?php  echo $this->session->flashdata('imgerror');?></span>
                     </div>
                     <input type="hidden" name="id" value="<?= $datas->id ?>">
                     <input type="hidden" name="old_img" value="<?= $datas->slides_image ?>">
                     <?php if($datas->slides_image ){?>
                        <div class="col-md-3 mt-4">
                           <?php $img =  'assets/images/no-image.jpg';?>
                           <?php if(!empty($datas->slides_image))  {?>
                              <?php 
                                 $img1 =  'assets/images/slider/'.$datas->slides_image; 
                                 if (file_exists($img1)) {
                                      $img = $img1;
                                 }
                              ?>
