	...
	<div class="card-body card-dashboard dataTables_wrapper dt-bootstrap">
		...
		<div class="row">
			...
			<div class="col-md-12 text-right">
				<?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/send',$permission)){ ?>
					<a href="<?php  echo base_url('admin/'.$url2.'/send'); ?>" class="btn btn-primary"> Send Notification</a>
				<?php } ?>
			</div>
		...
		<table id="table" class="table table-striped table-bordered file-export" data-toggle="table" data-pagination="false" data-search="true" data-show-columns="true" data-key-events="true" data-show-toggle="true"  data-cookie="true" data-cookie-id-table="saveId" data-show-export="false" data-click-to-select="false" data-toolbar="#toolbar">
			...
			<thead>
				...
				<tr>
					...
					<th style="max-width: 250px !important;">Title</th>	
                ...
                if (!empty($list)) {
                    ...
                    foreach ($list as $row) {
                        ...
                        <tr class="even"  data-val="<?=$row->id?>">
            				...
            				<td><?php echo !empty($row->title) ? $row->title : 'N/A' ?></td>

            				<td><?php echo !empty($row->description) ? $row->description : 'N/A' ?></td>

            				<td><?php echo !empty($row->device_id) ? count(json_decode($row->device_id)) : '0' ?></td>

            				<td><?php echo !empty($row->success_count) ? $row->success_count : '0' ?></td>

            				<td><?php echo !empty($row->error_count) ? $row->error_count : '0' ?></td>
