      ...
      <div class="col-md-12">
         <nav aria-label="breadcrumb">
            <ol class="breadcrumb pl-3 mr-3 ">
               ...
               <li class="breadcrumb-item active" aria-current="page">Stylist Expertise / Interests Details</li>
            </ol>
         </nav>
      </div>
   </div>
	...
	<div class="row">
		<div class="col-md-12 mt-3">
		   ...
		   <?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/add',$permission)){ ?>
				...
				<a href="<?php echo base_url('admin/add-stylist-expertise-interests'); ?>" class="btn btn-primary float-right "><i class="fa fa-plus" aria-hidden="true"></i> Add New Expertise / Interests </a>
			<?php } ?>
				...
				<tbody class="text-center">
                  ...
                  <td><button type="button" id="<?= $data->id; ?>" class="<?=$cls?> btn btn-sm mt-1 <?= ($data->status == 1)?"btn-primary":"btn-danger"; ?> ">
                    	...
                    	<?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/edit',$permission)){ ?>
                    		<a onclick="update_ui_order_('ui_order<?= $data->id?>',<?= $data->id?>,'area_expertise')" class="btn btn-primary"><i class="fa fa-refresh"></i></a>
                    	<?php }?>
                	</td>
                	<?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/edit',$permission) || in_array($url1."/".$url2.'/delete',$permission)){ ?>
							<td class=''>
								<?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/edit',$permission)){ ?>
							   		<a  class="btn btn-primary"  href="<?php echo base_url('admin/edit-stylist-expertise-interests/').$data->id;?>"><i class="fa fa-pencil-square-o text-warning1 fa-lg" aria-hidden="true"></i></a>
							   <?php }?>
							   <?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/delete',$permission)){ ?>
							   		<a class="btn btn-danger" href="<?php echo base_url('admin/delete-stylist-expertise-interests/').$data->id;?>"><i class="fa fa-trash text-danger1 fa-lg" aria-hidden="true"></i></a> 
							   <?php }?>
							</td>
						<?php }?>
   ...
   $(document).ready(function(){
	    	...
	    	$(document).on('click','.status_checks',function() {
	         ...
	         if(confirm("Are you sure to "+ msg)) {
	                  $.ajax({
	                  type:"POST",
	                  url: "<?= base_url('admin/update_stylist_expertise_interests_status'); ?>", 
