	...
	<div class="row">
		<div class="col-md-12 mt-3">
		    ...
		    <?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/add',$permission)){ ?>
			    <a href="<?php echo base_url('admin/add-our-services') ;?>" class="btn btn-primary float-right "><i class="fa fa-plus" aria-hidden="true"></i> Add New Service </a>
            <?php } ?>
					...
					<tr>
						...
						<td>
                    		...
                    		<?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/edit',$permission)){ ?>
                    			<a onclick="update_ui_order_('ui_order<?= $data->id?>',<?= $data->id?>,'fasishonServices')" class="btn btn-primary"><i class="fa fa-refresh"></i></a>
                    		 <?php }?>
                		</td>
						...
						<td class=''>
							<?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/edit',$permission)){ ?>
						   		<a  class="btn btn-primary"  href="<?php echo base_url('admin/edit-our-services/').$data->id;?>"><i class="fa fa-pencil-square-o text-warning1 fa-lg" aria-hidden="true"></i></a>
						   <?php }?>
						   <?php if($this->session->userdata('admin_id') == 1 || in_array($url1."/".$url2.'/delete',$permission)){ ?>
						   		<a class="btn btn-danger" href="<?php echo base_url('admin/delete-our-services/').$data->id;?>"><i class="fa fa-trash text-danger1 fa-lg" aria-hidden="true"></i></a> 
						   <?php }?>
						</td>
						<?php }?>
					</tr>
				   <?php $num++;  } } else {?>
				   <tr><td colspan="6" class="text-center">Our Services details not available.</td></tr>
				   <?php }?>
				</tbody>
			</table>
			</div>
      ...
      $('#example').DataTable();    
    	$(document).on('click','.status_checks',function() {
         ...
         if(confirm("Are you sure to "+ msg)) {
                  $.ajax({
                  type:"POST",
                  url: "<?= base_url('admin/update_our_services_status'); ?>", 
