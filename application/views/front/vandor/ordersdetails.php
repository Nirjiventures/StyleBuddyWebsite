					...
					<div class="rightbar">
						<div class="row">
							...
							<div class="col-sm-4 text-end">
								...
								&nbsp; - &nbsp; 
								<a href="<?=base_url('vendor/orders')?>" class="btn btn-success add_pro"><i class="fa fa-long-arrow-left" aria-hidden="true"></i> Back</a>
							</div>
						</div>
						<div class="summery_order">
							<div class="row">
								<div class="col-sm-9">
									<p class="mb-0"><b>Order ID : #<?= (!empty($order->id))? $order->id:'' ?> | </b>Payment Status: <span class="approved"><?= (!empty($order_detail[0]->payment_status))? ucwords($order_detail[0]->payment_status):'' ?></span>  | Status: <span class="approved"><?= (!empty($order_detail[0]->order_status))? ucwords($order_detail[0]->order_status):'' ?></span>  |  Order Date : <?= date('d F Y',strtotime($order->created_at)); ?></p>
								</div>
