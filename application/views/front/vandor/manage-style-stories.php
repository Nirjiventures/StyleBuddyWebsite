						...
						<table class="table table-bordered odc text-nowrap" id="example">
							...
							<?php if(!empty($blogs)) { foreach($blogs as $blog)  {  ?>
								<tr>
									...
									<td>
										<?php  if (file_exists($image_path = FCPATH . 'assets/images/story/' . $blog->blogImage)) { ?>
										    <img alt="Personal Styling | StyleBuddy"  alt="StyleBuddy"  src="<?= base_url('assets/images/story/'.$blog->blogImage) ?>"  class="order_pic">
										<?php  } else { ?>
