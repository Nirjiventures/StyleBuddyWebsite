<?php  $this->load->view('Page/template/header'); ?>
    ...
    <div class="container">
        <?php 
			...
			$this->breadcrumb->add($jobRow['job_title'], '/page/browsejobs');
	    ?>
        ...
        <div class="row justify-content-center  mt-5">
                    ...
                    <div class="row">
                        <div class="col-lg-12 col-md-12 col-xs-12">
                            ...
                            <!-- <div class="job-company-logo">
                               <img alt="Personal Styling | StyleBuddy"  alt="StyleBuddy"  src="<?php echo base_url(); ?>assets/images/logo.png" class="img-fluid" alt="">
                            </div> -->
                        </div>
                      </div>
                        <div class="col-lg-12 col-md-12 col-xs-12">
							<div class="job-details">
                                <h3><?= $jobRow['job_title']; ?> <span class="btn-full-time"><?= $jobRow['job_type']; ?></span></h3>
                                <div class="location"> 
                                	<?php if($jobRow['city']){ ?> 
							...
							<div class="share_icon pb-3">
									Share :
									<ul>
										<li><a  onclick="shareCount(<?=$jobRow['id']?>,'heartIconT')" class="twitter-share-button" target="_blank" rel="noopener noreferrer" href="https://twitter.com/intent/tweet?url=<?=base_url()?>page/browsejobdetail/<?=$jobRow['slug']?>"><img alt="Personal Styling | StyleBuddy"  alt="StyleBuddy"  src="<?php echo base_url(); ?>/assets/images/tw.png"></a></li>
										<li><a class="facebook-share-button" target="_blank" rel="noopener noreferrer" href="https://www.facebook.com/sharer/sharer.php?u=<?=base_url()?>page/browsejobdetail/<?=$jobRow['slug']?>"><img alt="Personal Styling | StyleBuddy"  alt="StyleBuddy"  src="<?php echo base_url(); ?>/assets/images/fb.png"></a></li>
										<!-- <li><a href="#"><img alt="Personal Styling | StyleBuddy"  alt="StyleBuddy"  src="<?php echo base_url(); ?>/assets/images/linke.png"></a></li> -->
																	...
																	<ul>
																		...
																		<?php if ($job_apply_num_rows < $nn) { ?>
																					<?php foreach ($job_apply as $key => $value) { ?>
																						<li><img alt="Personal Styling | StyleBuddy"  alt="StyleBuddy"  src="<?php echo base_url('assets/vandor/images/'.$value['job_apply_image']); ?>"></li>
																					<?php }?>
																		<?php }else{ $ppp = 0;?>
																					<?php foreach ($job_apply as $key => $value) { ?>
																						<?php if ($ppp < $nn) { ?>
							...
							<?php if ($this->session->userdata('userType')) {?>
								<?php if ($this->session->userdata('userType') == 2) {  ?>
										<?php if ($loginUserRow['profile_update_ratio'] >= 80) {?>
											<a data-bs-toggle="modal" data-bs-target="#job_appy_pop" onclick="post_job(<?=$jobRow['id']?>)" class="job_bt">SUBMIT YOUR PROFILE</a>
										<?php }else{ ?>
											<?php //$this->session->set_flashdata('success','<span class="text-danger p-2" style="font-size: 20px;    background: #ccc;"><b>Your profile needs to be at-least 80% complete to apply for the jobs at StyleBuddy. Please take a look at below mentioned parameters and complete that is pending and start applying for jobs.</b></span>'); ?>
											<a data-bs-toggle="modal" data-bs-target="#profile_compp" class="job_bt">SUBMIT YOUR PROFILE</a>
											<!-- <a href="<?=base_url('stylist-zone/dashboard')?>" class="job_bt">SUBMIT YOUR PROFILE</a> -->
										<?php }?>
								<?php }else{ ?>
									<?php $this->session->set_flashdata('festival_message','<span class="text-danger p-2" style="font-size: 20px;    background: #ccc;"><b>You are not allowed to apply this job.</b></span>'); ?>
									<a href="<?=base_url()?>" class="job_bt">SUBMIT YOUR PROFILE</a>
									<!-- <a data-bs-toggle="modal" data-bs-target="#please_login" class="job_bt">SUBMIT YOUR PROFILE</a> -->
								<?php }?>
							<?php }else{ ?>
								<a href="<?=base_url('login')?>" class="job_bt">SUBMIT YOUR PROFILE</a>
							<?php }?>
			...
			<!-- <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#profile_compp">Open modal</button> -->
