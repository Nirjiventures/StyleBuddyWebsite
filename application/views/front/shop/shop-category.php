						...
						<div class="col-sm-7 p-0">
							...
							<div class="fils_list_1">
								...
								<div class="row">
									...
									<?php if($parentCategory) { foreach($parentCategory as $value)  { ?> 
										
										<div class="col-md-12 mb-2"> <input type="checkbox" <?php if (in_array($value['id'], $catValArray)) { echo "checked";} ?> class="common_selector cat_id" name="catid[]" value="<?= $value['id'] ?>" > <label for="cat_id"><b style="color: #fc07a9;"> <?= $value['name'] ?> </b></label>  </div>
										
											<?php $j = 0;foreach ($value['child'] as $key1 => $value1) { ?>
							            		<?php if($value1['child']) { ?>
							            			<div class="col-md-4">
							            			    <li> <input type="checkbox" <?php if (in_array($value1['id'], $catValArray)) { echo "checked";} ?> class="common_selector cat_id" name="catid[]" value="<?= $value1['id'] ?>" > <label  for="cat_id"> <b><?= $value1['name'] ?> </b></label> </li><ul>
							            				
											                <?php foreach ($value1['child'] as $key2 => $value2) { ?>
											                	<li  class="label_3"> <input type="checkbox" <?php if (in_array($value2['id'], $catValArray)) { echo "checked";} ?> class="common_selector cat_id" name="catid[]" value="<?= $value2['id'] ?>" > <label for="cat_id"> <?= $value2['name'] ?> </label> </li>
															<?php } ?>
														</ul>
	    											    </div>	 
												<?php }else{ ?>
													<div class="col-md-4"> <input type="checkbox" <?php if (in_array($value1['id'], $catValArray)) { echo "checked";} ?> class="common_selector cat_id" name="catid[]" value="<?= $value1['id'] ?>" > <label for="cat_id"> <?= $value1['name'] ?> </label> 
													</div>
