...
<!-- Free session login function work start from here -->
<script type="text/javascript">
	function submitForm___(){
        $('#free_s_login').modal('hide');
        $('#free_s_message').modal('show');
    }

    function submitForm(){
		var userEmail = $('#FSessEmail').val();
		var userPassword = $('#FSessPass').val();
        if(userEmail == ''){
            $('#free_sess_msg').html('<span class="text-danger">Please enter username</span>');
            $('#FSessEmail').focus();
            return false; 
        }
        if(userPassword == ''){
            $('#free_sess_msg').html('<span class="text-danger">Please enter password</span>');
            $('#FSessPass').focus();
            return false; 
        }else{
        	$.ajax({
            ...
            data: {userEmail:userEmail,userPassword:userPassword},
            success: function(data) {
            	$('#free_sess_msg').html('<span class="text-danger">'+ data.response +'</span>');
            	if(data.status == 'success'){
            	    $.ajax({
	                type: 'POST',
	                dataType:"json",
	                url: '<?php echo base_url(); ?>login/loginDetails',
	                data: {userEmail:userEmail,userPassword:userPassword},
        	        success: function(data) {
	                    //console.log(data.response.name);
	                    if(data.status == 'success'){
	                        $("#freesess_name").val(data.response.name);
	                        $("#freesess_email").val(data.response.email);
	                        $("#freesess_mobile").val(data.response.mobile);
	                       }
	                    }
     	            });
            	...
            	//window.location.reload();
                    ...
                    $('#free_sess_msg').html('<span class="text-danger">Invalid email or Password, Try Again</span>');
	            }
		    }
	   }); 
        //$('#free_s_message').modal('show');
    ...
    function submitForm1(){
        $('#free_s_message').modal('hide');
        $('#free_s_popup').modal('show');
    }

    function submitForm2(){
        var book_sess_fname = $('#freesess_name').val();
        var book_sess_email = $("#freesess_email").val();
        var book_sess_mobile= $("#freesess_mobile").val();
        var book_sess_service  = $("#freesess_service").val();
        var book_sess_comment  = $("#freesess_comments").val();
        if(book_sess_fname == ''){
            $('#book_sess_now').html('<span class="text-danger">Please enter full name</span>');
            $('#freesess_name').focus();
            return false; 
        }else if(book_sess_email == ''){
            $('#book_sess_now').html('<span class="text-danger">Please enter Email</span>');
            $('#freesess_email').focus();
            return false; 
        }else if(book_sess_mobile == ''){
            $('#book_sess_now').html('<span class="text-danger">Please enter mobile</span>');
            $('#freesess_mobile').focus();
            return false; 
        }else if(book_sess_service == ''){
            $('#book_sess_now').html('<span class="text-danger">Please select services</span>');
            $('#freesess_service').focus();
            return false; 
        }else if(book_sess_comment == ''){
            $('#book_sess_now').html('<span class="text-danger">Please share something about yourself!</span>');
            $('#freesess_comments').focus();
            return false; 
        }else{
            $.ajax({
                type: 'POST',
                dataType:"json",
                url: '<?php echo base_url(); ?>login/bookFreeSess',
                data: {book_sess_fname:book_sess_fname,book_sess_email:book_sess_email,book_sess_mobile:book_sess_mobile,book_sess_service:book_sess_service,book_sess_comment:book_sess_comment},
                success: function(data) {
                    if(data.status == 'success'){
                       $('#book_sess_now').html('<span class=""></span>');
                       $('#free_s_popup').modal('hide');
                       $('#free_s_popup_message').modal('show');
                    }else{
                        $('#book_sess_now').html('<span class="text-danger">'+ data.response +'</span>');
                    }
	            }
                });
            }
