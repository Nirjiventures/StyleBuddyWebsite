...
class Home extends MY_Controller {
    ...
    public function ask_quote_lead(){   
        if ($this->input->post('phone') && $this->input->post('message')) {
            $data['message'] = $this->input->post('message');
            $data['phone'] = $this->input->post('phone');
             
            $insert =  $this->db->insert('ask_quote_lead',$data); 
            echo 'Thank you for contacting Style Buddy. We will get back to you shortly';      
        }
    }
    public function _404(){
        $data['seoData'] = $this->common_model->get_all_details('cms_pages',array('slug'=>'404-page-not-found','status'=>1))->row();
        $this->load->view('front/template/header',$data);
        $this->load->view('front/_404',$data);
    ...
    public function index(){ 
        ...
        $wh = ' WHERE parent_id = "'.$catid.'" order by id ASC';
        ...
        $expertises  =  $this->common_model->get_all_details_query('area_expertise_looking','where status = 1 order by ui_order asc')->row();
        $list = $expertises;
        ...
        $where .= "  order by experience desc limit 0, 8";
        ...
        $home_slider  =  $this->common_model->get_all_details_query('slider','where id = 17 AND status = 1')->row();
        $data['home_slider']  =  $home_slider;
        $data['home_slides']  =  $this->common_model->get_all_details_query('slides','where slider_id = "'.$home_slider->id.'" AND status = 1 ORDER by ui_order asc')->result();
        $data['home_slides_text']  =  $this->common_model->get_all_details_query('slides_text','where slider_id = "'.$home_slider->id.'" AND status = 1 ORDER by ui_order asc')->result();
         
        $data['section2']  =  $this->common_model->get_all_details_query('cms_pages','where slug="your-life-your-style"')->row();
        $data['section3']  =  $this->common_model->get_all_details_query('cms_pages','where slug="styling-service"')->row();
        $data['section4']  =  $this->common_model->get_all_details_query('cms_pages','where slug="styling-categories"')->row();
        $data['section5']  =  $this->common_model->get_all_details_query('cms_pages','where slug="our-customers-love-stylebuddy"')->row();
        $data['section6']  =  $this->common_model->get_all_details_query('cms_pages','where slug="daily-styling-tips-for-you"')->row();
        $data['section7']  =  $this->common_model->get_all_details_query('cms_pages','where slug="1000-qualified-professional-fashion-stylists-to-help-you-get-styled"')->row();
        $data['section8']  =  $this->common_model->get_all_details_query('cms_pages','where slug="stylist-curated-outfits"')->row();
        $data['section9']  =  $this->common_model->get_all_details_query('cms_pages','where slug="brand-partners"')->row();
        
        $table = 'home_page_services';
        $home_page_services = $this->common_model->get_all_details_query($table,' where status = 1 order BY ui_order limit 0,3')->result(); 
        $data['home_page_services'] = $home_page_services;
    ...
    public function up(){
        $rowsCatF = $this->common_model->get_all_details_query('products',array())->result_array();
        foreach ($rowsCatF as $key => $value) {
            $this->common_model->commonUpdate('products',array('recommended_count'=>rand(10,90)),array('id'=>$value['id']));
        }
        ...
        foreach($rows as $k=>$v){
            ...
            $a = array();
            $a['feedback_rating'] = $rows[$k]->feedbackRating;
            $brand = $this->common_model->commonUpdate('vender',$a,array('id'=>$v->id));
            echo $this->db->last_query();
        }
        $this->load->view('front/template/header',$data);
        $this->load->view('front/index',$data);
