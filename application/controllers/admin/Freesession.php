...
class Freesession extends MY_Controller {
    ...
    public function index(){
        $this->getPermission('admin/freesession');
        $table = $this->tbl_name;
        ...
        $data['list'] = $list;
        $this->load->view('admin/free_session/list',$data);
    }
    public function add(){ 
        $this->getPermission('admin/freesession/add');
        ...
        $this->load->view('admin/freesession/addedit',$data);
    }
    public function edit($id=''){
        $this->getPermission('admin/freesession/edit'); 
        $url1 = $this->uri->segment(1);
        $url2 = $this->uri->segment(2);
        $url3 = $this->uri->segment(3);
    ...
    public function freesess_export(){
        $filename = 'Book_free_session_'.date('Y-m-d-H-i-s').'.csv'; 
        ...
        $query = $this->db->query('SELECT full_name,email_id,mobile_no,services,sess_comments FROM '.$tbl_name.$str);
        $usersData = $query->result_array();
        $file = fopen('php://output', 'w');
        $header = array("Full Name","Email","Mobile","Services","Comments"); 
        fputcsv($file, $header);
    ...
    public function delete($id){
        $this->getPermission('admin/freesession/delete');
        //$table = $this->tbl_name;
        $table = 'book_free_sess';
        $delete = $this->common_model->commonDelete($table,array('id'=>$id));
        if($delete) {
            $this->session->set_flashdata('success','Free Session deleted successfully!!');
            redirect('admin/freesession');
        } else {
